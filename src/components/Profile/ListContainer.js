...
function ListContainerwithDropdown() {
  const [userlists, setUserlists] = useState([]);
  const [value, setValue] = useState("1");
  /* 
  async function getEntries(list_name) {
    const auth = firebase.auth();
    const user = auth.currentUser;
    const token = user && (await user.getIdToken());

    const books = await axios.get(
      `https://library-app-code.herokuapp.com/entries/:listname`,
      {
        list_name: list_name,
      },
      {
  ...
  /* 
  async function visualizeList(e) {
    <ListBrowser list_ID={e}></ListBrowser>;
  } */
...
export default ListContainerwithDropdown;
